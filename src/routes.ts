...
// Routes which are accessible to all.
export const publicRoutes: (string|RegExp)[] = ["/", "/auth/verify-email","/news","/about","/blogs","/contact","/entrechat","/events","/share-your-story" ,  /^\/blogs(\/.*)?$/,/^\/entrechat(\/.*)?$/ , /^\/news(\/.*)?$/,/^\/events(\/.*)?$/ ,"/about/core-team", "/gettingstarted" ];
